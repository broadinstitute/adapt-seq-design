...
def hyperparam_grid():
    ...
    """
    grid = {
            'conv_num_filters': [5, 10, 15, 20, 25],
            'conv_filter_width': [[1], [2], [1, 2], [1, 2, 3, 4]],
            'pool_window_width': [1, 2, 3, 4],
    ...
    """
    grid = {
            'conv_num_filters': [10, 15],
            'conv_filter_width': [[2], [1, 2]],
            'pool_window_width': [2],
            'fully_connected_dim': [[40]],
            'pool_strategy': ['max'],
            'locally_connected_width': [[1]],
...
def search_for_hyperparams(x, y, num_splits=5, loss_out=None):
...
def nested_cross_validate(x, y, num_outer_splits=5, num_inner_splits=5,
        loss_out=None):
    ...
    if args.command == 'hyperparam-search':
        ...
        if args.params_mean_val_loss_out_tsv:
            params_mean_val_loss_out_tsv_f = open(
                    args.params_mean_val_loss_out_tsv, 'w')
        else:
