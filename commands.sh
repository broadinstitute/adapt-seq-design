...
# Select model ea838c02 from the above, which has the lowest MSE
# Another reason model ea838c02 looks good: in the ranking of
#   MSE it is ranked first; in the ranking of 1_minus_rho
#   it is ranked third. If we average the rankings of MSE and
#   1_minus_rho for all models, model ea838c02 ranks highest.
#   (The one downside is that it has high SEM for its 1_minus_rho
#     values, but most models do.)

# Create test results
python predictor.py --load-model models/predictor_exp-and-pos_regress-on-active/model-ea838c02 --dataset cas13 --cas13-subset exp-and-pos --cas13-regress-only-on-active --context-nt 10 --write-test-tsv out/cas13-hyperparam-search.exp-and-pos.regress-on-active.model-ea838c02.test.tsv.gz --seed 1 &> out/cas13-hyperparam-search.exp-and-pos.regress-on-active.model-ea838c02.test.out

# Plot test results
Rscript plotting_scripts/plot_predictor_test_results.R out/cas13-hyperparam-search.exp-and-pos.regress-on-active.model-ea838c02.test.tsv.gz out/cas13-hyperparam-search.exp-and-pos.regress-on-active.model-ea838c02.test.pdf
